...
const PLAN = {
  monday: {
    ...
    exercises: [
      ...
      {id:'m11', name:'Upright row (cable, wide grip)', sets:3, reps:'10-12', note:'Different movement pattern (vertical pull) — hits medial delt + traps. Wide grip keeps wrists neutral'},
  ...
  wednesday: {
    label: "Chest — Upper & Lower Focus",
    exercises: [
      {id:'w2', name:'Incline DB press', sets:3, reps:'10-12', note:'Upper chest — do first while fresh. Alternate with decline as your lead lift week to week'},
      {id:'w8', name:'Decline DB press', sets:3, reps:'10-12', note:'Lower chest / the "lip" — sub feet-elevated flat press if your bench doesn’t decline'},
      {id:'w1', name:'Flat DB bench press', sets:4, reps:'8-10', note:'Overall mass'},
      {id:'w3', name:'Cable fly (low-to-high)', sets:3, reps:'12-15', note:'Upper chest angle'},
      {id:'w4', name:'Cable fly (high-to-low)', sets:3, reps:'12-15', note:'Lower chest angle'},
...
// ---------- storage helpers (Supabase "kv" table) ----------
  ...
  for(let i=1;i<=ex.sets;i++){
    ...
    rows += `
      <div class="set-row" data-set="${i}">
        ...
        <input type="text" inputmode="numeric" placeholder="${ex.reps}" value="${esc(s.r||'')}" data-field="r">
...
function buildNutritionPanel(){
  ...
  div.innerHTML = `
    ...
    <details class="block">
      <summary><h3>Portion cheat sheet</h3></summary>
    ...
    <details class="block">
      <summary><h3>Training day template</h3></summary>
    ...
    <details class="block">
      <summary><h3>Rest day template</h3></summary>
...
function buildProgressPanel(){
  ...
  div.innerHTML = `
    ...
    <div class="weigh-form">
      ...
      <input type="number" step="0.1" inputmode="decimal" class="wi-weight" placeholder="weight (lb)">
...
// ---------- wiring ----------
// Builds the app inside `root`; returns a cleanup function.
export function startApp(root){
  ...
  nav.querySelectorAll('button').forEach(btn=>{
    btn.addEventListener('click', async ()=>{
      ...
      // center the tapped tab in the scrolling tab row, and start the new tab at its top
      nav.scrollTo({ left: btn.offsetLeft - (nav.clientWidth - btn.offsetWidth)/2, behavior: 'smooth' });
      const panelTop = main.offsetTop - nav.offsetHeight;
      if(window.scrollY > panelTop) window.scrollTo({ top: panelTop });
